#!/usr/bin/env bash
#
# vfio-wrapper.sh - Wrapper to run vfio-toggle.py detached from user sessions.
...
TARGET="$DIR/vfio-toggle.py"

if [[ ! -x "$TARGET" ]]; then
  # Fallback to PATH if not in the same directory
  TARGET="$(command -v vfio-toggle.py || true)"
fi

if [[ ! -f "$TARGET" ]]; then
  echo "Error: Cannot find vfio-toggle.py" >&2
  exit 1
fi
...
echo "Launching vfio-toggle.py as a detached system service..."
